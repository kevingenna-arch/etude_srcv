...
charger <- function(annee, cfg) {
  ...
  if (!is.null(cfg$naiss_ind)) {                     # FPR : naissance reconstruite
    ni <- cfg$naiss_ind
    nm <- lire_srcv(cfg$ind) %>%
      transmute(.cle = trimws(as.character(.data[[ni$cle_ind]])),
                ne = as.integer(num(.data[[ni$annais]]) %in% ni$annees)) %>%
      group_by(.cle) %>% summarise(y_birth = as.integer(any(ne == 1, na.rm = TRUE)),
                                   .groups = "drop")
    raw <- raw %>% mutate(.cle = trimws(as.character(.data[[ni$cle_men]]))) %>%
      left_join(nm, by = ".cle") %>% mutate(y_birth = replace_na(y_birth, 0L))
  } else raw <- raw %>% mutate(y_birth = as.integer(num(.data[[v[["y_birth"]]]]) == 1))
...
qcut <- function(x, w, k, labs, nom = "") {
  ...
  seuils <- x[o][sapply(seq_len(k-1)/k, function(p) which(cw >= p)[1])]
...
w <- d %>% filter(if_all(c(statut3, urbain), ~ !is.na(.))) %>% pull(poids)
w <- d$poids[as.integer(rownames(acm))]
