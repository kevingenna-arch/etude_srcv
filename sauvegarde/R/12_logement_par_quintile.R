...
brut <- imap_dfr(VAGUES[ANNEES], ~ charger(.y, .x))
...
res <- svyby(~surface + cout_m2 + effort, ~annee + classe + statut3 + quintile,
             des, svymean, na.rm = TRUE) %>%
  as_tibble() %>%
  left_join(count(d, annee, classe, statut3, quintile, name = "n"),
            by = c("annee", "classe", "statut3", "quintile"))

# ── Évolutions 2006 -> 2025 ─────────────────────────────────────────────────
evo <- res %>%
  select(annee, classe, statut3, quintile, surface, cout_m2, effort, n) %>%
  ...
  arrange(classe, statut3, quintile)

if (!dir.exists("Output")) dir.create("Output")
write_csv(evo, "Output/logement_par_quintile.csv")
